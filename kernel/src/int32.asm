...
;   int32(0x10, &regs);
; 
; 2013/10/14 Modified by domitry:
;   add codes turning off paging
;   add some macros preparing to turn off paging
;   modified some constant value to fit Kiwi's memory map
;
...
%define INT32_BASE                             0x5022
%define REBASE(x)                              (((x) - reloc) + INT32_BASE)
%define GDTENTRY(x)                            ((x) << 3)
%define DATA32                                 GDTENTRY(1)	; 0x08	(modified)
%define CODE32                                 GDTENTRY(2)	; 0x10  (modified)
%define CODE16                                 GDTENTRY(3)	; 0x18
%define DATA16                                 GDTENTRY(4)	; 0x20
%define STACK16                                (INT32_BASE - regs16_t_size)

%define VIR2PHYS(x)				(x-0xc0000000)	; by domitry
...
section .text
	...
	_int32:
		...
		; added by domitry from here
		sub  esp,0xc0000000 			; move stack pointer

		jmp  VIR2PHYS(physical)			; jump to physical address
	physical:
		mov  eax,cr0
		and  eax,0x7fffffff
		mov  cr0,eax				; disable paging
		; added source up to here

		mov  esi, VIR2PHYS(reloc)              ; set source to code below
		...
		jmp eax                         ; absolute offset jump to new code location
	reloc: use32                               ; by Napalm
		...
		sub  esi,0xc0000000			; added by domitry
	...
	p_mode32: use32
		...
		sub  edi, 0xc0000000			; added by domitry
		mov  ecx, regs16_t_size                ; set copy size to our struct size
		cld                                    ; clear direction flag (so we copy forward)
		...
		mov eax,virtual				; absolute offset jump to label "virtual"
		jmp  eax
	virtual:
		add  esp,0xc0000000
		; added by domitry up to here
