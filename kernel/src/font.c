...
_Bool drawFontApp(UCHAR *buffer,int x,int y,int f_width,char ch,Color col){
	int i;
	char offset = ch - f_info.first;
	CharHeader *ch_h = f_info.ch_start + offset;
	UCHAR *r_seek,*w_seek,*r_end;
	...
	drawImage2App(buffer,f_info.buffer,x,y,ch_h->width,ch_h->height,f_width,IMG_RGBA);
	return 1;
}


_Bool drawStringApp(UCHAR *buffer,int x,int y,int f_width,const char *str,int color){
	int i=0,cur_x=0,num=0,height=0;
	CharHeader ch;
	Color col = {(UCHAR)(color>>16),(UCHAR)(color>>8),(UCHAR)color};
	...
	for(i=0;i<num;i++){
		...
		ch = f_info.ch_start[str[i]-f_info.first];
		drawFontApp(buffer,cur_x,height-ch.height,f_width,str[i],col);
		cur_x += ch.width + 2;
	}
	return 1;
}
